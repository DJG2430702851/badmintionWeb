<template>
	<div id="noticedetail" class="noticedetail">
		...
		<div class="pn clearfix">
			<div class="prev"><上一篇:<span v-if="prevData.newsId" @click="goUrl('/nod/'+prevData.newsId)">{{prevData.newsSubject}}</span><font v-else>无</font></div>
			<div class="next">>下一篇:<span v-if="nextData.newsId" @click="goUrl('/nod/'+nextData.newsId)">{{nextData.newsSubject}}</span><font v-else>无</font></div>
		</div>
  </div>
</template>
<script>
import {getNoticeDetail,getNoticeLastAndNext} from '@/service/newsData'
...
export default {
	...
	mounted(){
	  	let vthis = this;
	  	getNoticeDetail(this.listParam.id).then((data) => {
	  	...
	  	getNoticeLastAndNext(this.listParam.id).then((data) => {
